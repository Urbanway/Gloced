...
set   :ip,                  "64.207.179.116"
set   :domain,              "gloced.com"
set   :subdomain,           false # unless domain is a subdomain, e.g. subdomain.domain.com
set   :user,                "root"
set   :keep_releases,       5
# This configuration is *conventional*
set   :application,         domain
set   :deploy_to,           "/home/rails/#{domain}"
set   :repository_path,     "gloced.git"
set   :repository,          "git@bitbucket.org:flakerimi/#{repository_path}"
...
namespace :deploy do
  ...
  task :createsessions do
    puts "\n\n=== Creating the Production Database! ===\n\n"
    run "cd #{current_path}; rake db:sessions:create RAILS_ENV=production"
    system "cap deploy:set_permissions"
  end
  ...
  namespace :gems do
    ...
    task :install do
      if ENV['gem']
        ...
        run "cd #{current_path}; bundle update RAILS_ENV=production"
      end
    end
  ...
  namespace :nginx do
    ...
    task :create do
      ...
      system "rsync -vr tmp/#{domain} #{user}@#{application}:/usr/local/nginx/conf/vhost/#{domain}"
      File.delete("tmp/#{domain}")
...
#
## main details
#set :application, "gloced.com"
#role :web, "gloced.com"
#role :app, "gloced.com"
#role :db,  "gloced.com", :primary => true
#
## server details
#default_run_options[:pty] = true
#ssh_options[:forward_agent] = true
#set :deploy_to, "/www/gloced.com"
...
#set :repository, "git@github.com:flakerimi/Gloced.git"
#set :branch, "master"
